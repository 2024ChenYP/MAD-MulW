...
output_dir = r"./"                   # 保存路径
train_dataset = torch.load(f'../data/Epilepsy/before/train.pt')
test_dataset = torch.load(f'../data/Epilepsy/before/test.pt')
val_dataset = torch.load(f'../data/Epilepsy/before/val.pt')

trainsamples = train_dataset["samples"].squeeze(1).numpy()  # 样本
trainlabels = train_dataset["labels"].long().numpy()  # 标签
testsamples = test_dataset["samples"].squeeze(1).numpy()  # 样本
testlabels = test_dataset["labels"].long().numpy()  # 标签
valsamples = val_dataset["samples"].squeeze(1).numpy()  # 样本
vallabels = val_dataset["labels"].long().numpy()  # 标签

data = np.concatenate((trainsamples, testsamples), axis=0)
sample = np.concatenate((data, valsamples), axis=0)
labels = np.concatenate((trainlabels, testlabels), axis=0)
labels = np.concatenate((labels, vallabels), axis=0)
...
for i in range(0, n-1):
    plt.plot(range(len(data[domain_down:domain_up])), data[domain_down:domain_up, i], label=f'character{i + 1}', alpha=0.7)
